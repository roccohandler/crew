...
// items page keeps the bar for its back chevron. Twin of web components/nutrition/ChainPicker.tsx. WRITTEN — UNVERIFIED (needs Mac).
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: EmberTokens.Spacing.space16) {
                    HStack(alignment: .firstTextBaseline, spacing: EmberTokens.Spacing.space12) {
                        ...
                        TextActionButton(title: "Cancel", role: EmberTokens.Typography.textButton) { onPick(nil) }
                    }
                    VStack(spacing: 0) {
                        ForEach(Array(model.chains.enumerated()), id: \.element.id) { index, chain in
                            ...
                            NavigationLink {
                                ...
                                HStack(spacing: EmberTokens.Spacing.space12) {
                                    Image(systemName: chain.icon).foregroundStyle(EmberColors.ink).accessibilityHidden(true)
...
struct ChainItemsList: View {
    ...
    var body: some View {
        ...
        .toolbarRole(.editor) // the back button is the bare chevron
    }
    ...
    private func row(_ item: SeedFastFoodItem) -> some View {
        ...
        return Button { onPick(item) } label: {
            HStack(spacing: EmberTokens.Spacing.space8) {
                ...
                Spacer(minLength: EmberTokens.Spacing.space8)
                Text(numerals: MacroDay.gramsText(grams)).typeRole(EmberTokens.Typography.secondary).foregroundStyle(EmberColors.ink).lineLimit(1)
            }
