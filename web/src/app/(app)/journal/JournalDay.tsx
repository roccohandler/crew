...
type Props = { dayKey: string; todayKey: string; dayPosts: PostDoc[]; trainingDays: TrainingDaysEntry[]; sessionLines: Map<string, string> };

export function JournalDay({ dayKey, todayKey, dayPosts, trainingDays, sessionLines }: Props) {
  ...
  return (
    <section className="stack stack--tight" aria-label={label}>
      <h3 className="row row--between">
        ...
        {isRestDay(dayKey, dayPosts, trainingDays) ? <span className="whisper">Rest day</span> : null}
      </h3>
      {dayPosts.map((post) => (
