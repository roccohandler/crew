...
export interface WeekRecord { weekKey: string; done: number; planned: number; sets: number; meals: number }
...
export async function progressFacts(userId: ObjectId, timezone: string, plannedWeekdays: number[], now: Date = new Date()) {
  ...
  for (let offset = RING_WEEKS - 1; offset >= 0; offset -= 1) {
    const weekKey = addDays(weekKeyFor(todayKey), -offset * TimeUnits.daysPerWeek);
    ...
    weeks.push({ weekKey, done: new Set(weekSessions.map((session) => session.dayKey)).size, planned: plannedWeekdays.length, sets: weekSessions.reduce((sum, session) => sum + session.exercises.flatMap((exercise) => exercise.sets).filter((set) => set.done && !set.isWarmup).length, 0), meals: ownPosts.filter((post) => post.type === "meal" && inWeek(post.dayKey)).length });
  }
  const balance = { push: 0, pull: 0, legs: 0, fullBody: 0 };
  for (const session of completed) {
    const name = session.workoutName.toLowerCase();
    if (name.startsWith("push")) balance.push += 1; else if (name.startsWith("pull")) balance.pull += 1; else if (name.startsWith("leg")) balance.legs += 1; else balance.fullBody += 1;
  }
  return { todayKey, days, weeks, balance, totals: { workouts: completed.length, posts: ownPosts.length, longestStreak: state?.longestStreak ?? 0, currentStreak: state?.currentStreak ?? 0 }, strength: strengthTrends(completed) };
}
