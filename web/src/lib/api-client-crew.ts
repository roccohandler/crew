...
// A5/A6: a stream post carries its summary line; A7: blocked list, unblock, notification preferences, profile photo. A21.2 / W3
// (2026-09-17): the chat is gone — no sendMessage, no deleteMessage; a stream item is a post or a system line; renameCrew is the
// Captain's PATCH from the Invite panel (E2).
...
export const renameCrew = async (crewId: string, body: { name?: string; emoji?: string }) => (await patchJson(`/crews/${crewId}`, body)) as { crew: { id: string; name: string; emoji: string } };
