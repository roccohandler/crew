"use client";
// SPEC: nutrition addendum §4 (RATIFIED 2026-09-18) · 6.9 Screen Density (A25 — pass/fail) — Today has ONE job: the four lines
// (P / C / F and Calories) and "Your template", where one tap logs a slot and the same tap undoes it. Quick add and today's log are
// each one tap away behind a labelled button (TodayLinks; R-077), and the text link leads to Saved meals & template. NO ink-filled
// primary of its own (Q3: the Home row "Log macros" is the way in). The slot write is optimistic and rolls back on failure, written
// out here (C5): the tick appears at once, the server's copy replaces it, an error takes it away again and says so in ink — never in
// the semantic red, which does not exist on this surface. A log is never a game event (clause ③): nothing here reads or moves XP, the
// streak or a shield. Twin: ios Features/Nutrition/NutritionTodayScreen.swift.
import { useRef, useState } from "react";
import { MacroLines } from "@/components/nutrition/MacroLines";
import { TemplateRows, TodayLinks } from "@/components/nutrition/TodayParts";
