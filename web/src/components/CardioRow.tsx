...
export const unitLabel = (units: "lb" | "kg") => (units === "kg" ? "km" : "mi");
...
export function metersFrom(text: string, units: "lb" | "kg"): number | null {
  const value = Number(text.trim());
  if (text.trim() === "" || !Number.isFinite(value) || value <= 0) return null;
  const perUnit = units === "kg" ? SpecConstants.metersPerKilometer : SpecConstants.metersPerMile;
...
export function CardioFields({ name, defaultMinutes, units, action, disabled, onSubmit }: { name: string; defaultMinutes: number; units: "lb" | "kg"; action: string; disabled: boolean; onSubmit: (seconds: number, distanceMeters: number | null) => void }) {
  ...
  return (
    <div className="stack stack--tight">
      ...
      <label className="field field--short">
        <span>Distance ({unitLabel(units)}, optional)</span>
      ...
      <button type="button" className="button button--primary" disabled={disabled} onClick={() => onSubmit(minutes * TimeUnits.secondsPerMinute, metersFrom(distance, units))}>{action}</button>
    </div>
  );
}

export function CardioRow({ name, seconds, distanceMeters, units, done, onDone }: { name: string; seconds: number; distanceMeters: number | null; units: "lb" | "kg"; done: boolean; onDone: (seconds: number, distanceMeters: number | null) => void }) {
  const minutes = Math.round(seconds / TimeUnits.secondsPerMinute); // rounded like the server's summary line
  const distance = done && distanceMeters !== null ? ` · ${distanceText(distanceMeters, units)}` : "";
  return (
    <div className="stack stack--tight">
      <p className="muted">{name} · {minutes} min{distance}{done ? " ✓" : ""}</p>
      {done ? null : <CardioFields name={name} defaultMinutes={minutes} units={units} action="Done" disabled={false} onSubmit={onDone} />}
    </div>
  );
}
