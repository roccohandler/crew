...
describe("lib/db — Part IX invariants as unique indexes", () => {
  ...
  it("one plan per user", async () => {
    ...
    await collection.insertOne({ _id: new ObjectId(), userId, trainingWeekdays: [], trainingDaysHistory: [], workouts: [], updatedAt: new Date() });
    await expectDuplicateKey(() => collection.insertOne({ _id: new ObjectId(), userId, trainingWeekdays: [], trainingDaysHistory: [], workouts: [], updatedAt: new Date() }));
  });
