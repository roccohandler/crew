...
test("signed-in pages: landmarks, names, labels, no sideways scroll", async ({ page }) => {
  ...
  for (const path of ["/home", "/plan", "/crew", "/progress", "/journal", "/settings", "/session/new", "/plan/push", "/log-cardio", "/nutrition", "/nutrition/meals", "/nutrition/template", "/nutrition/targets", "/nutrition/method", "/how-crew-works"]) await expectAccessible(page, path); // A22: /post is gone · W8: the nutrition pages (first-run state)
});
