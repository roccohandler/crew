...
| Method + path | Schema | Does | Errors |
|---|---|---|---|
| GET `plans` | — | the user's single Plan — `PlanResponse { trainingWeekdays: number[], trainingDaysHistory: [{ from, weekdays }], workouts: [WorkoutTemplate], updatedAt }` (A1; A27 (a): the history — each entry in effect from its dayKey `from`, append-only, `trainingWeekdays` always its last entry; a plan from before A27 is migrated on its first read to one entry in effect from its creation dayKey): `trainingWeekdays` are the ISO weekdays (1 = Mon … 7 = Sun) the user trains, sorted; `workouts` is the ORDERED rotation, one per `kind` ∈ push \| pull \| legs \| fullBodyA \| fullBodyB \| custom, each `{ kind, name, exercises: [ExerciseTemplate] }` — no weekday on a workout: the rotation assigns days (the next workout is the one after the last completed rotation workout). A document written before A1 (one workout per weekday) reads back normalised: `trainingWeekdays` = its weekdays, workouts deduped by kind in first-seen order, `weekday` dropped; it is rewritten on the next PUT | `notFound` (never created) |
| PUT `plans` | `putPlanSchema` { trainingWeekdays: [1..7, unique, 1–7 entries], workouts: [1–7 × { kind, name, exercises: [ExerciseTemplateInput] }], savedAt?: ISO instant } — `kind` unique across the list, order kept as given | replace the whole plan, forward-only (existing sessions keep their snapshots); A27 (a): a change of `trainingWeekdays` is APPENDED to the history, in effect from the day it was saved — `savedAt` (a queued phone edit) when it lies inside E15's window, else now — never before the last entry; the same days append nothing; enforces `planMaxExercisesPerDay` (15), `planMaxSetsPerExercise` (20), `exerciseNameMaxChars` (60); `ExerciseTemplateInput.type` ∈ strength \| mobility \| cardio — mobility holds AND cardio blocks carry `holdSeconds` (seconds, A2: ≤ `holdSecondsMax` for a hold, ≤ `cardioMinutesMax` × 60 for a cardio block) | `validation` |
