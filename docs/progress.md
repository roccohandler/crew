...
- NEXT (cold resume): read docs/OWNER-REVIEW.md first, then docs/testing-without-a-mac.md (the owner has no Mac). The repo is on GitHub (`roccohandler/crew`, public) and the compile-fix loop runs on GitHub's macOS runners: the owner runs `& "C:\Program Files\Git\bin\bash.exe" C:/Users/princ/CREW_2.0/docs/commit-queue.sh` then `git push`; the agent pulls the failing step's log with `gh api repos/roccohandler/crew/actions/jobs/<id>/logs` (job ids from `…/actions/runs/<run>/attempts/<n>/jobs`), fixes, queues the next block. State 2026-09-08 12:30Z: the GitHub billing lock is LIFTED (a card update paid the past-due balance); CI run 34224403865 attempt 3 = contracts · web · engine-swift · web-e2e GREEN on GitHub; the `ios` job failed BEFORE compiling — the image (Xcode 26.6) has no "iPhone 16" simulator, so both CI files now pick the newest plain iPhone at run time (block S38). The first Xcode compile of the SwiftUI half is the NEXT push; the desk-check of 2026-09-05 (R-042) and the Linux build (R-048) removed the errors a blind read can find — expect the ones they cannot. Then credentials (T045–T047).
