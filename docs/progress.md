...
- ⏳ UI-TOUR ORDER (owner, 2026-09-18, "closed visual feedback loop": screenshot tour · SessionStart fetch hook · design/DESIGN.md · ui-reviewer · /ui-check · /approve-screens) — STOPPED BEFORE ANY FILE WAS WRITTEN, on the owner's own term ("conflict → SPECIFICATION GAP and stop"). Six gaps await a ruling, proposed as A24: (1) a LOCAL-FIXTURE demo mode cannot fill the Crew tab without a seam in `Api` (C2/C3/C4; CrewApp's "no test-only path in the app") — proposal: the tour seeds the CI harness through the real API (SeedClient), as 8.4 already does; (2) "macros" fixtures = building W8 ahead of its gate; (3) CLAUDE.md is "paste verbatim" (5.4) — the @design/DESIGN.md import + the /ui-check rule need a 5.4 amendment; (4) DESIGN.md brief is stale against the registry (#FF6600 → #DF5908 by A17.4; "light as the face" → LIGHT ALWAYS by A21.10) and "the density principle" is nowhere in the spec; (5) the tour duplicates the ios-storyboard (owner order 2026-09-18 item 1) with a second naming scheme and a second macOS job per UI push; (6) the verify step deletes a branch, which the git rule + hook forbid. Nothing else in the order conflicts; it is ready to build the moment these are ruled.
