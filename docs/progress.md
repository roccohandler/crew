...
Updated: 2026-09-08 evening (cold-start full audit, then gap closures Q01–Q10) · Current phase: 5 → 6 (everything runnable
...
- [~] T024 Home S07 — WRITTEN-UNVERIFIED (CI: HomeModelTests + HomeModelEdgeTests green; bridge and post-state screenshots reviewed R-053)
- [~] T025 Session S09 — WRITTEN-UNVERIFIED (CI: SessionModelTests green; journey ② logs 3/3 sets)
...
- [~] T031 Crew feature S12–S13 (iOS) — WRITTEN-UNVERIFIED (CI: journey ② sees the crew-mate's reaction; Crew screenshot reviewed R-053)
...
Phase 5 — hardening
- [~] T040 Progress + Journal (S15–S16) — PARTIAL: web DONE-VERIFIED (build + e2e a11y sweep incl. /journal); iOS WRITTEN-UNVERIFIED
- [~] T041 Settings (S17): pause, export, delete cascade — PARTIAL: routes DONE-VERIFIED (account.test.ts: export completeness, cascade crawl, pause) + web settings page (e2e sweep); iOS WRITTEN-UNVERIFIED
...
- [~] Q11 Deploy check — host = `crew-eta-one.vercel.app`; DONE 2026-09-08 ~18:50Z: `/api/v1/users/me` → 401 and `/` → 200 (after the Root Directory fix, redeployed via `npx vercel redeploy`); REMAINING: the owner adds MONGODB_URI / JWT_SECRET / CRON_SECRET / MODERATION_INBOX + connects the Blob store, the agent redeploys, then ONE journey at ONE viewport against production (`BASE_URL=https://crew-eta-one.vercel.app npx playwright test journey1 --project=desktop-1280` — the auth limit is 10/min/IP, so never the whole matrix from one address) and deletes the test account afterwards (DELETE users/me, confirm "delete"). [SPEC: XI T046; OWNER-REVIEW §5 step 6]
...
- NEXT: the owner runs the commit queue (blocks F08–F13) and pushes; the agent reads the CI run (`gh run list --repo roccohandler/crew --limit 1`, then `gh run view <id>`) — the two new UI probes (Q09) and the C10 split (Q08) meet a compiler there for the first time; fix without changing behaviour if red. Then Q11 (the Vercel host → curl 401/200 → Playwright against production), then the owner's Stage 2 steps (docs/testing-without-a-mac.md). The server is complete to the spec's route list and tested (`npm test` 31 files / 309 green incl. every route group, events, and the 139 standing checks).
