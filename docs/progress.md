...
- 2026-09-17 (late, after A22): the owner said GO on all remaining work. W3 (crew surface, A21.2) and W4 (invite code, push opt-in, two-button share, photo prompt) are DONE on master locally, every local gate green, NOT pushed — the owner wants W3 → W6 pushed together. NEXT: W5, W6, then ONE push, the CI verdict (the iOS app target's first compile of W3 + W4), one smoke test. W3b (plate-journal removal) waits on G1–G4; W7 on the owner's accounts; W8 on the addendum's ratification; W9 on the privacy/terms text. W2's build 136 still waits on the certificate revocation. Build B stays on `archive/a20-build-b` until the W6 walkthrough decides rebuild-or-drop (A21.12). The nutrition addendum draft awaits ratification (W8).
