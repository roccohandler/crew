...
- **Build.** Expected **206** (commit count). CI, TestFlight and the ui-reviewer verdict on the Home shots (light and dark, against `design/targets/01`–`06`) are recorded in the next commit, because no commit is docs-only.
