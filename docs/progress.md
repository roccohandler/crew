...
- 2026-09-17 (late, after A22): the owner said GO on all remaining work. W3 (crew surface), W4 (invite code, push opt-in, two-button share, photo prompt), W5 (private photos, bound Apple callback, Secure cookies, report resolution, deletion leftovers) and W6 (walkthrough fixes; A21.12 pending the owner) are DONE on master locally, every local gate green. NEXT: the ONE push, the CI verdict (the iOS app target's first compile of W3–W6 — read it before anything else), one smoke test; then W3b on G1–G4, W7 on the owner's accounts, W8 on the addendum, W9 on the privacy/terms text. W3b (plate-journal removal) waits on G1–G4; W7 on the owner's accounts; W8 on the addendum's ratification; W9 on the privacy/terms text. W2's build 136 still waits on the certificate revocation. Build B stays on `archive/a20-build-b` until the W6 walkthrough decides rebuild-or-drop (A21.12). The nutrition addendum draft awaits ratification (W8).
