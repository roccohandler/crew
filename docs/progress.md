...
## 2026-09-19 — A28 BUILD ORDER COMPLETE: R1–R7 BUILT, EVERY SCREEN THROUGH ITS THREE REVIEWER ROUNDS

- **Shipped.** Home, the Logger, Progress, Plan, Crew + Manage crew, Settings (grouped) and Nutrition are on the Focus Card system on iOS, light and dark. The web's parity is noted per screen in docs/debt.md and was not built (the build order's rule).
- **Builds.** TestFlight **217** (6863e61) and **221** (169038d) uploaded after green CI. d71ff46 (run 35450242203) was red on one web e2e test alone (journey ⑤ desktop, `read ECONNRESET` from the test server; no web code changed), with iOS green; 169038d re-ran everything green. The last commit's CI is recorded in the next entry, or it is the commit's own run.
- **Reviewer verdicts, last round per screen.** Home round 3: every Home state PASS, sheets fixed forward (R-094). Logger round 3: 7 PASS · 2 FAIL, both fixed forward unreviewed (R-096). Progress + Plan round 3: 6 PASS · 0 FAIL. Crew + Settings + Nutrition round 3: 10 PASS · 1 FAIL, fixed forward unreviewed (R-096).
- **Owed.** The residue is in docs/debt.md ("A28 review / residue after three rounds", "A28 R1 / Home residue after three rounds"), plus the owner's calls: GAP 6 (macro colours), GAP 9 (the celebration phrase), GAP 10 (season inputs), the pulse and dots, the Journal's XP sentence, Blocked people's CTA, the disabled-primary outline (R-092 (1)), avatars. Baselines are NOT approved (the owner's /approve-screens).
- **Next.** The owner's device pass on build 221 or later, then the owner's rulings on the open GAPs; the web parity session.
...
- **R5–R7 reviewer verdicts (run 35445082374).** Crew round 1: 0 PASS · 8 FAIL. Settings round 1: 5 PASS · 7 FAIL (Profile, the photo dialog, the paused Home light and dark, How Crew works passed). Nutrition round 1: 1 PASS · 13 FAIL (the Home macros row passed).
- **This commit (R-092).** Crew round 2 and Settings round 2 fixed forward: the disabled primary as an outline, the bottom bar without its rule on the 20 pt gutter, Crew's own title with text-button Manage and Invite, the stream scrolled to its newest post, the create and join sheets on the system, Manage crew without stacked labels, Settings' own title, Pause with one title and a stating line. Owed to the owner: Blocked people's empty CTA, the pulse and the dots (GAP), the avatars.
- **Next commit (R-093).** Nutrition round 2: Saved meals and the Template as two screens, the kit's check on the template, no stacked field labels (units as words: "180 lb", "40 g"), the gram rows and every list in cards on the 20 pt gutter, text-button actions, the meal form on the system's sheet, the sources without underlines.
- **6863e61: run 35447860873 GREEN; TestFlight build 217** (run 35449255197). ui-reviewer on its tour: Home round 3 + the Logger round 2, 17 PASS · 13 FAIL (every Home state passed; the FAILs were the "+" sheet, bonus sheet, Log cardio, reminder sheet, the Logger's fact-line inset, the whole-workout sheet's height, the holds' check position). Progress + Plan round 2, 9 PASS · 6 FAIL (the Journal, the week map's lines, the editor's Save, the exercise sheet's header).
- **f7d5b8a: run 35448570159 red on one flaky test** (HomeStatesTests: the bridge sampled before the reinstall pull landed; it passed on 6863e61). Its tour went to ui-reviewer: Crew round 2 5 PASS · 3 FAIL, Settings round 2 8 PASS · 1 FAIL, Nutrition round 2 6 PASS · 7 FAIL. Fixed forward in R-095 (next commit), with the test made to wait.
- **Next commit (R-094).** All of those fixed forward. Home has had its three rounds, so its residue is in docs/debt.md. Then the run on f7d5b8a (Crew, Settings and Nutrition round 2) and this commit's run (the Logger round 3, Progress and Plan round 3).
