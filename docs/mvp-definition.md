...
| W4 | Invite code (A21.3) + the web hero passing a real token + push registration (A21.4) + two-button share (A21.9) + the profile-photo prompt at first crew join (1C) | L | iOS journey ③ via a pasted code lands in-crew; the web hero routes to /join/[token]; permission asked once after the first completed workout and a push-token row exists; a test proves no post document exists before a celebration button is tapped; the photo prompt appears at first join |
