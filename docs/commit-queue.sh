...
# --- F29 (run 34373681818: the signed build compiles and tests; the signup password field kept one typed character — Automatic Strong Password on a .newPassword field) ---
...
# --- F39 (the owner: "why didn't I see the new app update on test flight — I always want to see the update when I push code that passes") ---
commit_task "feat(ci): a green CI run on master now builds to TestFlight by itself — nothing ever connected the two, so the last upload was the commit BEFORE A17 and the whole Home redesign reached no phone while three runs went green. The trigger is workflow_run on ci, gated to conclusion == success and head_branch == master, because workflow_run fires on every completion including failures and pull-request runs. Three things a workflow_run job gets wrong by default and does not here: it checks out github.event.workflow_run.head_sha rather than the default branch's tip, so the build is the code that actually went green and not whatever landed while it queued; the build number is git rev-list --count HEAD with fetch-depth 0, because App Store Connect rejects anything not higher than the last upload for the same version string and that is not a thing a human should have to remember; and a testflight concurrency group with cancel-in-progress stops two quick pushes racing two uploads. Manual dispatch survives with build_number now optional. A closing step writes the number, the commit and the fact that App Store Connect still has to PROCESS the build for a few minutes before the phone can see it — the other half of why don't I see the update [SPEC: XI T045; Part X Phases 6-7; docs/testing-without-a-mac.md]

Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>" \
  .github/workflows/testflight.yml docs/testing-without-a-mac.md docs/progress.md docs/commit-queue.sh

# --- F40 (the owner ratified A18 in sixteen answers and A19 as drafted; the paperwork lands before the code it governs) ---
# NOTE ON ORDERING: docs/progress.md and docs/commit-queue.sh are claimed by F39 above, which runs first, so the A18
# and A19 sections of progress.md ride with that commit. The queue stages whole files; it cannot split one across two.
commit_task "docs(spec): A18 and A19 enter the Decision Registry — the fourth Home review and the app-wide ergonomics audit. A18.1-A18.13 answer the owner's four questions about the shipped rest-day Home with thirteen clauses and a ruling that the pass needs NO vector (the suite is the gamification fixture contract; pause-aware week marks are a reporting function that awards nothing, and V57-V64 stay reserved for A16). A19.1-A19.6 ratify the audit as drafted with three owner rulings carried in: R4 takes shape (b), two buttons rather than a toggle, which is specifically the shape that keeps post visibility OUT of the completion inputs and therefore needs no vector either; the Journal becomes a segment inside Progress rather than a sixth tab; and the order is A18 tail, then A19 Stage A, then Stages B and D, then A15, then Stages C, E and F — because A18 settled Home's CONTENT and A19.1 settles its ANCHORING, and every element A18 added is what brings the anchor's collapse closer. Part VII's S07 row and Part III's token table move with them [SPEC: Appendix A A18; Appendix A A19; S07; Part III; 6.5; 6.7; XI T024]

Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>" \
  docs/crew-mvp-spec.md docs/home-plan-a18-2026-09-10.md docs/ios-ux-audit-2026-09-10.md docs/a19-handoff-prompt.md docs/debt.md

# --- F41 (A18: the owner photographed the rest-day Home A17 produced and asked four questions; none of them had an answer on screen) ---
commit_task "feat(home): A18 — Home answers the four questions it was raising. The ring said '1/3' with the word 'workouts' existing only in a VoiceOver label, beside a flame whose '1' meant something else entirely, so the natural reading of the header was wrong; both numerals are named where they sit now. The ring itself was printing '0/3' on every non-bridge MONDAY — the identical zero-as-verdict that got it removed from the bridge, one day in seven, on six states nobody had checked — so it renders only once the week holds a completed workout. The rest card stated its reward and never its premise, which is why 'recovery is part of the plan' and a Post a meal button read as a contradiction: V04 is titled 'rest day, silence -> streak 0 at 3 AM', and the card says so now. The three 'strange divs' were real buttons wearing a stat-tile grammar (a NOUN over a verb) inside a 1.26:1 border against a 3:1 gate, in the silhouette Apple defines as a segmented control; they are three full-width verb rows. The void gets the what's-next fact as its own block. Plus: the paused screen stops contradicting itself (pause-aware marks, no ring, 'End the pause now', and the web pause guard iOS had and web never mirrored), every planned day gets a mark so the strip can be read against the ring, the bridge carries one CTA again when a session is open, the all-done card reports the day in the journal's own sentence, the toolbar camera goes where the card already asks, controlOutline becomes a token of its own, and Home's offline state becomes REACHABLE rather than merely declared. Twelve pure defects ride with it, including two false source comments that would have been quoted against the next decision. Tests: today-state.ts had ZERO of them and now has ten; the layout gate runs at 375x667, 393x852 and 440x956 instead of the one size where the defect does not occur; the a11y sweep audits a real Home instead of the bridge; ShellStatesTests asserts reachability instead of counting a five-element literal; and CI photographs all four non-bridge states [SPEC: Appendix A A18; S07; 1D; A3; A8; A14; A17; V04; V20; 6.3; 6.5; 6.6; 6.7; 8.9; E6; E20; Ember (1) (3) (4); XI T024/T036/T040/T043]

Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>" \
  shared/design-tokens.json ios/Crew/Generated/EmberColors.swift web/src/generated/ember.css \
  ios/Crew/Features/Home ios/Crew/Shared/PrimaryButton.swift ios/Crew/Shared/WeeklyRing.swift \
  ios/Crew/Storage/Store.swift ios/Crew/Storage/SyncQueue.swift ios/Crew/Features/Settings/SettingsModel.swift \
  ios/CrewTests/HomeModelTests.swift ios/CrewTests/HomeModelFactsTests.swift ios/CrewTests/HomeModelEdgeTests.swift \
  ios/CrewTests/HomeVectorSlotsTests.swift ios/CrewTests/HomeTestFixtures.swift ios/CrewTests/ShellStatesTests.swift \
  ios/CrewUITests/HomeStatesTests.swift ios/CrewUITests/SeedClient.swift .github/workflows/ci.yml \
  web/src/app web/src/components web/src/lib web/tests

# --- F42 (A19 Stages A, B and D: the anchor was never real, the numeric keyboards had no way out, and the only action on three screens sat at the top) ---
commit_task "feat(ios): A19 Stages A/B/D — the bottom anchor becomes real. safeAreaInset had ZERO uses in the app: every 'bottom-anchored' primary was a Spacer inside scrolling content, which collapses to nothing the moment content exceeds the viewport, or a VStack sibling that simply scrolls away, or an overlay with hand-computed padding guessing the home indicator's height differently in each file. A18's new layout gate measured the consequence on the rest-day Home — 212 px of slack at 440x956, and 24 px at 375x667 only BECAUSE the page already overflows and the anchor has already collapsed — so 6.7's 'primary actions stay bottom-anchored regardless of how much canvas exists above' has never been mechanically true anywhere, only true while each screen happened to fit. One new modifier, .crewBottomBar, over Apple's API for exactly this, conditional so a state with no primary gets no bar (A17.3, A18.9), adopted lowest-risk-first on the generated plan, the nutrition post, the cardio log, the session, login and signup. The keyboard half comes free, and the rest is a Done item: .numberPad and .decimalPad ship no return key, so the cardio distance field and the birth-year field could be focused with no way to dismiss the keyboard and no way to reach the button beneath it — the only escape threw the entry away. Stage D inverts three spacers that put the only action on a screen at the TOP with dead canvas beneath it (the exact defect A17.2 fixed on Home and nowhere else), adds ScrollView overflow valves the SE needs at accessibility-XXL, and moves 'Discard workout' out of the session's scroll tail where 6.3 had it sitting directly above Complete at the moment the user reaches for it. A18.11's token sweep finishes here: nine more control boundaries were still drawing at 1.26:1, and contrast.test.ts now sweeps every Swift file against a five-entry allowlist of the surfaces still entitled to a hairline. Home and the workout editor deliberately do NOT adopt the bar yet — A19.1 ratifies 'Home last', and the editor's primary lives in the nav bar, which makes moving it R11 [SPEC: Appendix A A19; R1; R2; R3; R5; R6; R7; R9; R10; A18.11; 6.3; 6.5; 6.7; E5; Ember (1); XI T021/T022/T025/T027/T041]

Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>" \
  ios/Crew/Shared/BottomBar.swift ios/Crew/Shared/TextActionButton.swift ios/Crew/Features/Session/SetCountButton.swift \
  ios/Crew/Features/Onboarding ios/Crew/Features/Post ios/Crew/Features/Session ios/Crew/Features/Plan \
  ios/Crew/Features/Crew/PostCard.swift ios/Crew/Features/Settings/EditProfileScreen.swift \
...
# --- F48 (run 35069768536: the app COMPILES — 0 errors, 140 tests ran — and all five failures were test defects) ---
commit_task "fix(test): five failing tests on the first ios run since 2026-09-10, and not one of them was the app. F47's revert worked: 0 compile errors, 140 unit tests executed, web and ios-engine green — the first real evidence A18, A19 and A20 Build A are sound. This job had not run a test since 334ad67, so three passes' worth of test code executed at once and two never-exercised defects surfaced together. (1) HomeModelFactsTests expected 'Push day' and got 'Leg day · 15/15 sets · 0 min' — and this is NOT the date-dependence of F04, it is ORDER-dependence the source already documents: NextUpLine.swift:75 says 'A1: a SwiftData to-many keeps no order of its own', every production reader obeys it (HomeModel.swift:71 and PlanModel.swift:48 select by kind, NextUpLine.swift:30 sorts by order), and FIVE test sites called bare .first on LocalPlan.workouts while one then asserted a day NAME. It drew Leg day on this runner; it was never reliable, it had simply never been run. All five pinned .first { \$0.kind == 'push' }, the idiom HomeModelEdgeTests.swift:24 already used. (2) Three journeys died on 'Neither element nor any descendant has keyboard focus' at Password — one field EARLIER than the field A20.9 had just fixed. A20.9's diagnosis was right (A19.1's safeAreaInset and A19.2's keyboard toolbar animate against the edge typeText synthesises against) but it guarded the race with 'if text.allSatisfy(isNumber) && !app.keys[1]', which covers Birth year and nothing else; a SecureTextField raises no digit to wait for, so Password kept the bare tap-then-type. Build A fixed one FIELD, not the CLASS. The helper's own note is right that app.keyboards proves nothing between two text fields — but hasKeyboardFocus on the field ITSELF does, whatever keyboard it raises, so that is the general proof now and the digit wait stays for the numberPad's extra teardown. All five S05 fields route through typeInto, and so does the nutrition caption: NutritionPostScreen.swift:35 adopts .crewBottomBar (A19.1), giving it the identical animating edge, and since the last executed run predates A19 it had never once run with the bar present — it would have been the next red. The sixth failure, a screenshot TIMEOUT rather than an assertion, is deliberately NOT patched: it shared a run with three focus failures, so a wedged simulator cannot be told from a real hang, and making shoot() non-fatal would blind the one mechanism this repo has for looking at the app from Windows. Debt records it for the next run. Seven test files, zero app files. NO VECTOR CHANGES [SPEC: Appendix A A20.11; A1; A19.1; A19.2; 8.4; 5.3; C5; XI T024/T028/T035/T043/T008]

Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>" \
  ios/CrewTests/HomeModelFactsTests.swift ios/CrewTests/AchievementsLocalTests.swift \
  ios/CrewTests/HomeModelTests.swift ios/CrewUITests/JourneySteps.swift \
  ios/CrewUITests/CameraDeniedTests.swift ios/CrewUITests/Journey1_NewUserTests.swift \
  ios/CrewUITests/OfflineSessionTests.swift docs/debt.md docs/progress.md docs/commit-queue.sh

# --- F49 (run 35073421853: 140 unit tests GREEN; the xcresult was read and named all three remaining failures) ---
commit_task "fix(test): the last three UI failures, diagnosed from the xcresult instead of guessed at — two stale assertions and one race that found a real defect. F48 worked: 140 unit tests, 0 failures, zero keyboard-focus errors anywhere, and HomeStatesTests passed, which retires the screenshot timeout as collateral from a wedged simulator exactly as its debt entry was opened to determine. T-A3's entry demanded the artifact be read if red persisted, so it was: 112 MB downloaded, zstd blobs decompressed on Windows, and the trace settles the old question — on Password the first tap never took focus (hasKeyboardFocus polled t=33.3 to t=37.3 and never went true), the retry tap at t=37.59 logged 'Scroll element to visible', focus arrived at t=40.17. The race was REAL and the field was genuinely obscured; A20.9's diagnosis was right and only its guard was numeric-only. (1) OfflineSession called a lost session on a screen whose hierarchy dump reads: StaticText 'Today', StaticText 'Your first flame lights today.', Button 'Resume your first workout'. The session was never lost. A18.8 says it in the source (TodayCard.swift:24-26) — the bridge lasts until the first POST, starting a workout is not a post, so an abandoned first workout used to put a Resume banner NEXT TO the bridge's CTA, two prompts on the one screen 1D says carries none, and the bridge's single button resumes instead; HomeScreen.swift:65 suppresses the banner with !isBridge and TodayCard.swift:110 renames the CTA. The assertion was accusing the app of a bug A18 had deliberately designed away, in a failure message a reader would have believed. (2) Journey1 queried app.buttons['Skip'], which matched the bare title until A19 (cb84202) gave the control a real VoiceOver name, 'Skip <exercise>' (SessionScreen.swift:115). E20 wants a control that says what it skips; the app is right and the query was stale, and it had simply never run — the last executed ios job, 334ad67, predates A19 by a day. (3) The journeys tapped Save 0.34 s after typeText began on Birth year and the save read the binding early: the dump shows the field holding 1994 beside the server's 'birthYear: Too small: expected number to be >=1900', which is what Int('19') earns. The fix is the flow A19.2 built its Done bar FOR — saveThePlan(in:) taps Done, which resigns first responder and COMMITS the field, before Save; third occurrence, so a plain function (C5), and the only test anywhere that exercises that bar. THAT RACE FOUND A REAL DEFECT AND IT IS RECORDED, NOT SMUGGLED IN: SaveAuthScreen.swift:84 validates with Int(birthYear) == nil ? 'Four digits, like 1994.' : nil, so '19' raises no client error, passes the :88 guard, and the user reads the server's raw sentence under the field — the message promises four digits and the check enforces neither four digits, nor the 1900 floor, nor E9's 13+ age floor. A signup validator is an app behaviour change and this pass exists to green the CI, so debt.md carries it for its own commit. Four test files, zero app files. NO VECTOR CHANGES [SPEC: Appendix A A20.11; A18.8; A19.2; E9; E20; 1D; S07; S09; 8.4; C5; XI T024/T025/T028/T035/T043/T008]

Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>" \
  ios/CrewUITests/JourneySteps.swift ios/CrewUITests/CameraDeniedTests.swift \
  ios/CrewUITests/Journey1_NewUserTests.swift ios/CrewUITests/OfflineSessionTests.swift \
  docs/debt.md docs/progress.md docs/commit-queue.sh

# --- F50 (the real defect F49's diagnosis uncovered, repaid in its own commit as the owner ruled) ---
commit_task "fix(auth): the birth-year check accepted numbers its own message says it will not, and the user read the SERVER's sentence for it. SaveAuthScreen asked only whether the text PARSED — Int(birthYear) == nil — so '19' raised no client error, passed submit()'s guard and was POSTed, and the field then rendered 'birthYear: Too small: expected number to be >=1900' verbatim under a label promising 'Four digits, like 1994.' (run 35073421853's hierarchy dump, found while diagnosing a UI-test race). Both clients now mirror the SERVER'S OWN TWO BOUNDS from the generated constants: >= SpecConstants.birthYearMin, the floor validate.ts birthYearSchema sets, and utcYear - year >= SpecConstants.minimumAgeYears, the gate users.ts requireSignupGates applies. Those two subsume the schema's max, so no third check and no number typed inline. MEASURED RATHER THAN ASSUMED: each old validator disagreed with the server on SIXTEEN years — 1898-1899 and 2014-2027 — and iOS additionally accepted '19'; web had the four-digit shape and neither bound. web/tests/onboarding-birth-year.test.ts pins the PARITY and not the wording: it sweeps every year from before the floor to past today and asserts the client accepts exactly the set requireSignupGates accepts, against a FIXED clock, because a fixture that drifts with the calendar is the shape that went red on a Friday twice here (F04, F43). Web is 43 files / 453 tests. Two judgements worth keeping: NO NEW COPY was invented — both sentences were already approved and already on that screen, so this changes a check and not the voice — and iOS reads the year from DayKey in UTC rather than the local calendar, because requireSignupGates counts against getUTCFullYear() and a local year running BEHIND UTC would make the client STRICTER than the server and refuse someone who had just turned 13. NO VECTOR CHANGES: the server's behaviour is untouched and this is a client mirroring a rule that already existed, the same shape as validatePassword mirroring passwordMinChars [SPEC: E9; S05; A20.11; C7; 8.2; XI T022/T036]
